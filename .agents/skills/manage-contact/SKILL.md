...
name: manage-contact
description: >
  Create or update a Contact note in the Obsidian vault with a dynamic Interactions section
  powered by Obsidian Bases. Always creates both the .md contact file and the corresponding
  _Meetings.base file. Use when asked to add a new contact or update contact information.
---
...
### 2. Derive Filename
Convert the contact name to a vault-safe filename: `Firstname_Lastname` (no diacritics in filename, underscores for spaces). Example: `Jan Palecek` -> `Jan_Palecek.md`.
